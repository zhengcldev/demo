package com.algorithm.demo.algorithm.SFU_CE;
...
/**
 * This is an implementation of the "SFU-CE algorithm" for mining skyline frequent utility itemsets
...
public class SFU_CE {
    ...
    int popSize = 2000; // the sample numbers
    int proSize = 0;//pro_size is the number of based on probability
    final int iter = 2000; // the maximum iterations of algorithm
    int transCount = 0; // the pro_size of transactions
    Integer cusItem;//the item whose utility is CUS
    int acIter = 0;//the actual iterations for statistics
    int CUS = 0;//the critical utility of SFUIs
    int fMax = 0;//the maximal frequency of 1-itemset
    final double alpha = 0.2;// the quantile parameter
    final double beta = 0.3; // the mutation parameter
    Map<Integer, Integer> mapItemToU;// a map that stores the utility of each item
    ...
    List<Integer> twuPattern;// the items which has twu value more than CUS
    double[] PV; // probability vector

    // this inner class represent an item ,its frequency and its utility in a transaction
    static class Pair {
        int item = 0;
        int utility = 0;
        int frequency = 0;
    ...
    class Particle {
        BitSet IV; // the sample vector
        int frequentFitness; // fitness value of sample
        int utilityFitness;// fitness frequency of sample
    ...
    //Create a list to store a transaction
    List<Item> Items;
    // Create a list to store in CSFUIList
    List<Particle> CSFUIList = new ArrayList<>();
    // Create a list to store SFUIs
    ...
    public void runSFU_CE(File input) throws IOException {
        // reset
        maxMemory = 0;
        //get SysTime
        ...
        generatePop();
        //this parameter is to record the difference between the maximum utility and the minimum utility
        int max_min;
        //start iterating
        for (int i = 0; i < iter; i++) {
            ...
            //max_min is the difference of the maximum and the minimum in the population based on probability
            max_min = population.get(0).utilityFitness - population.get((int) ((1 - beta) * population.size()) - 1).utilityFitness;
            // stopping criterion
            if (max_min == 0) {
                break;
            }
    ...
    /**
     * this method to calculate the CUS
     *
    ...
    /**
     * This is the method to generate the initialized population
     */
    private void generatePop() {
        ...
        for (i = 0; i < popSize - 1; ++i) {
            ...
            //create a list to record the collection of transactions in which the itemset resides
    ...
    /**
     * This is the method to update the probability vector and generate the new sample
     */
    private void update() {
    ...
    /**
     * judge the temp particle ,if it is not dominated
     * then inset it into sfuiParticleList
     *
    ...
    private void judge(Particle temp) {
        ...
        CSFUIList.removeIf(tempSFUI -> (tempSFUI.frequentFitness < temp.frequentFitness && tempSFUI.utilityFitness <= temp.utilityFitness) ||
    ...
    public boolean isRBAIndividual(Particle tempParticle, List<Integer> list) {
        ...
        //if tempBitSet.cardinality() == 0 then suggest that not exists any itemset like tempBitset in database
    ...
    public AlgoResult getAlgoResult() {
        ...
        algoResult.setAlgoName("SFU-CE");
        algoResult.setSFUI(String.valueOf(SFUIList.size()));
