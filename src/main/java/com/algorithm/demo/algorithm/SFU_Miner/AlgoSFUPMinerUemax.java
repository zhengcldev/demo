package com.algorithm.demo.algorithm.SFU_Miner;
...
public class AlgoSFUPMinerUemax {
	...
	public void runAlgorithm(File input) throws IOException {
		// reset maximum
		maxMemory =0;
		...
		SFUPMiner(new int[0], null, listOfUtilityLists, psfupList, uEmax);
	 ...
	 */
	private void SFUPMiner(int[] prefix, UtilityList pUL, List<UtilityList> ULs, SkylineList[] psfupList, int[] uEmax) {
		...
		for(int i=0; i< ULs.size(); i++){
			...
			if(X.sumIutils + X.sumRutils >= uEmax[temp] && uEmax[temp]!=0){	
				...
				SFUPMiner(newPrefix, X, exULs, psfupList, uEmax);
			}
		}
	}
